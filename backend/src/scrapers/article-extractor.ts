/** Fetch raw article text from a URL, prioritising the semantic article body */
...
/** Extract readable prose from raw text for use as a non-AI summary fallback */
export function extractReadableSummary(text: string, title: string): string {
  // Skip to first sentence boundary after 100 chars to avoid breadcrumb remnants
  let body = text;
  if (text.length > 300) {
  ...
  const junkPattern =
    /cookie|javascript|©|all rights reserved|skip to|privacy policy|subscribe|newsletter|sign up|log in|sign in/i;
  ...
  // Return up to 7 good sentences (~600-900 chars)
  let result = '';
